...
class DrawingUtil {
    ...
    static drawSquareBridge(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (var j = 0; j < 2; j++) {
            ...
            context.fillRect(size * (1 - sf2) + w * 0.5 * (sf3 - 1) , -size / 2, size * sf2, size)
            
            for (var i = 0; i < 2; i++) {
                context.save()
                context.translate(0, -size / 2 + size * i)
...
class SBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new SBNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawSBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : SBNode {
        var curr : SBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
